...
import com.issueTracker.auth.constants.MAXIMUM_DB_CONNECTION_PULL_SIZE
...
private fun initializeDatabase() {
    if (RoleEntity.findById(com.issueTracker.auth.constants.Roles.USER_ROLE_ID) == null) {
        RoleEntity.new {
            name = "user"
    ...
    if (RoleEntity.findById(com.issueTracker.auth.constants.Roles.ADMIN_ROLE_ID) == null) {
        RoleEntity.new {
            name = "admin"
    ...
    if (UserEntity.findById(com.issueTracker.auth.constants.Users.ADMIN_USER_ID) == null) {
        UserEntity.new {
            firstName = "admin"
            lastName = "admin"
            email = "admin@domain.com"
            passwordHash = BCrypt.hashpw("admin", BCrypt.gensalt())
            role = RoleEntity.findById(com.issueTracker.auth.constants.Roles.ADMIN_ROLE_ID)!!
        }
    }
}
